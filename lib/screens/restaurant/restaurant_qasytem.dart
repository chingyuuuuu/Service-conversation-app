...
3.add未回答問題的router-目的為了讓user可以看那些資料不在database
進而去新增這些資料到database中
4.處理基本問答
 */
...
class RestaurantQASystemState extends State<RestaurantQASystem> {
  ...
  void _sendMessage() {
    ...
    if (_controller.text.isNotEmpty) { //要記得設置listen:false
      final qaProvider = Provider.of<QAprovider>(
          context, listen: false); // 將 listen 設為 false
  ...
  Widget build(BuildContext context) {
    final qaProvider = Provider.of<QAprovider>(context); // listen 設為 true
    final speechProvider = Provider.of<SpeechProvider>(context); // 管理錄音狀態
    return Scaffold(
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      GestureDetector(
                        onTap: () => speechProvider.startRecording(context, _controller, () => _sendMessage(),), // 點擊時調用錄音對話框
                        child: CircleAvatar(
                          radius: 30,
                          backgroundColor: Colors.white,
                          child: Icon(
                            Icons.mic,
                            color: speechProvider.isRecording ? Colors.red : Colors.black,
                          ),
                        ),
                      ),
                    ],
